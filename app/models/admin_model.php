...
function add($question,$answere1,$answere2,$answere3,$answere4,$answere){      
    ...
    function resultforall(){
        $stmt = $this->_dbconnect->prepare("SELECT * FROM `score`");
         $stmt->execute();   
        $row = $stmt->fetchAll(PDO::FETCH_ASSOC); 
        
        for ($i = 0; $i<$stmt->rowCount();$i++){
            
            $resultscore = $this->generate_result($row[$i]['id']);
            
            $stmt2 = $this->_dbconnect->prepare('UPDATE `score` SET `score` = :score WHERE `id` = :user_id');      
            $stmt2->execute(array(':score' => $resultscore,':user_id' => $row[$i]['id']));
    ...
    {   
        ...
        {   $i=0;
            ...
            do{
               if($row[$i]['answer']==$this->generate_result2($row[$i]['qid']))
               {
                   $this->correct+=1;
               }            
                $i++;
...
function generate_result2($qid)
  {
    ...
    function display(){
         ...
         $stmt = $this->_dbconnect->prepare("SELECT * FROM `score` ORDER BY name ASC");
         $stmt->execute();   
        $row = $stmt->fetchAll(PDO::FETCH_ASSOC); 
